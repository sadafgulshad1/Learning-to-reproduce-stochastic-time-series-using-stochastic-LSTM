...
"""
lstm_layer1 = tf.nn.rnn_cell.LSTMCell(input_dim*2)
#The LSTM state as a Variable initialized to zeroes
print input_layer
print lstm_layer1
lstm_output1, lstm_state_output1=tf.nn.dynamic_rnn( cell=lstm_layer1, dtype=tf.float32, inputs=input_layer)
...
"""
lstm = tf.nn.rnn_cell.LSTMCell(input_dim*2)
lstm_state_output1 = state = tf.zeros([1, lstm.state_size])
...
output_W2 = tf.Variable(tf.truncated_normal([input_dim*2, input_dim]))
output_b2 = tf.Variable(tf.zeros([input_dim]))


#Compute the output
final_output1 = tf.matmul(lstm_output1, output_W1) + output_b1
final_output2 = tf.matmul(lstm_output1, output_W2) + output_b2

# print lstm_output1
# mean_lstm = tf.tanh(final_output1)
mean_lstm = final_output1
var_lstm = tf.exp(final_output2)
# print mean_lstm
# print var_lstm
# final_output = tf.add(mean_lstm, var_lstm)
# print final_output

##Input for correct output (for training)
correct_output = tf.placeholder(tf.float32, [1, input_dim])
#applying drop out
#mean_lstm = tf.nn.dropout(mean_lstm , dropout)
##Calculate the Sum-of-Squares Error
# error = tf.pow(tf.sub(final_output, correct_output), 2)
temp = tf.pow(tf.sub(mean_lstm, correct_output), 2)
temp1 = tf.div(temp, -2 * var_lstm)
error = tf.div(tf.exp(temp1), tf.sqrt(2*pi*var_lstm))


##The Optimizer
#Adam works best
train_step = tf.train.AdamOptimizer(0.0006).minimize(-tf.log(error))
#train_step = tf.train.AdamOptimizer(0.000001).minimize(-error)
#train_step = tf.train.GradientDescentOptimizer(0.0006).minimize(-error)
...
for i in range(iter):
    ...
    _, _, network_output, _ = sess.run([lstm_update_op1,
                                     train_step,
                                     mean_lstm,
                                     var_lstm],
                                     # final_output],
...
for i in range(10000):
    ...
    _, network_output,_ = sess.run([lstm_update_op1,
                                  mean_lstm,
                                  var_lstm],
                                  # final_output],
                                 feed_dict = {
